...
export default function AchievementsSection() {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-300 dark:border-neutral-800">
      {/* Header */}
      <div className="flex items-center justify-between bg-gradient-to-r from-[#4C0E0F] to-[#7a1a1c] px-4 py-3">
        ...
        <Link
          ...
          className="flex items-center gap-0.5 text-xs text-white/70 transition-colors hover:text-white"
        >
      ...
      <div className="flex flex-col gap-4 p-4">
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex items-center gap-4 rounded-xl bg-gray-50 p-3 dark:bg-neutral-800/50">
              <CircularProgress pct={totalPct} size={52} strokeWidth={4} color="#4C0E0F">
                <span className="text-xs font-black text-[#4C0E0F] dark:text-red-400">
                  {totalPct}%
                </span>
              </CircularProgress>
            ...
            {nextUp.length > 0 && (
              <div>
                <div className="mb-2 flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-bold tracking-wide text-gray-500 uppercase dark:text-gray-400">
                    Next Up
                  </span>
                </div>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            {isClosest && (
                              <span className="shrink-0 rounded-full bg-orange-500 px-1.5 py-0.5 text-[9px] font-bold text-white">
                                Closest
                              </span>
                            )}
                          </div>
                          ...
                          <p className="mt-0.5 text-[10px] text-gray-400">
                            {pct}% · {(a.target - a.progressValue).toLocaleString()} to go
                          </p>
                        </div>
                      </div>
                    )
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {recentUnlocked.map((a) => (
                    <div
                      key={a.id}
                      className="flex items-center gap-2.5 rounded-xl border border-gray-200 bg-white p-2.5 dark:border-neutral-800 dark:bg-neutral-900/50"
                    >
                      ...
                      <span className="shrink-0 rounded-full bg-green-100 px-1.5 py-0.5 text-[9px] font-bold text-green-700 dark:bg-green-900/40 dark:text-green-400">
                        ✓
                      </span>
                    </div>
                  ))}
            ...
            <Link
              href="/dashboard/achievements"
              className="flex items-center justify-center gap-1.5 rounded-xl border border-[#4C0E0F]/20 bg-[#4C0E0F]/5 py-2 text-xs font-semibold text-[#4C0E0F] transition-colors hover:bg-[#4C0E0F]/10 dark:border-red-400/20 dark:bg-red-950/20 dark:text-red-400"
            >
              <Trophy size={12} />
