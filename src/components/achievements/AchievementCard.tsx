      ...
      className={`group relative flex items-center gap-4 rounded-2xl border p-4 transition-all duration-300 ${
        achievement.unlocked
          ...
          : nearComplete || isEmailHighlight
            ? `border-gray-200 bg-white shadow-sm hover:border-gray-300 hover:shadow-md dark:border-neutral-800 dark:bg-neutral-900 dark:hover:border-neutral-700`
            : 'border-gray-200 bg-gray-50 opacity-80 hover:opacity-100 dark:border-neutral-800 dark:bg-neutral-900/50'
      } ${highlight ? 'animate-pulse ring-2 ring-[#4C0E0F]/30 ring-offset-1 dark:ring-red-400/30' : ''}`}
    >
      {achievement.unlocked ? (
        <div className="relative shrink-0">
          ...
          <CheckCircle2
            size={18}
            className="absolute -right-1 -bottom-1 rounded-full bg-white text-green-500 dark:bg-neutral-900"
            fill="currentColor"
          />
        </div>
      ) : (
        <div className="shrink-0">
          ...
          >
            <span
              className={`text-xl ${pct === 0 ? 'opacity-40 grayscale' : nearComplete ? '' : 'opacity-50'}`}
            >
              {achievement.emoji}
            </span>
          </CircularProgress>
