...
export function PublicNoteCard({ note, onClick }: PublicNoteCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className="group flex flex-col min-[375px]:flex-row items-start min-[375px]:items-center justify-between rounded-[12px] sm:rounded-[16px] md:rounded-[20px] border border-[#C9C9C9] dark:border-neutral-700 bg-[#FFFBFB] dark:bg-[#2A2A2A] p-3 sm:p-4 md:p-6 hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors w-full min-h-[80px] sm:min-h-[91px] gap-3 min-[375px]:gap-0 cursor-pointer"
        >
            <div className="flex items-center gap-3 sm:gap-4 md:gap-6 flex-1 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                    <h3 className="text-sm sm:text-base md:text-[20px] font-medium text-gray-900 dark:text-white truncate">{noteTitle}</h3>
            ...
            <div className="flex flex-row min-[375px]:flex-col items-center min-[375px]:items-end gap-2 min-[375px]:gap-1 flex-shrink-0">
                <p className="text-xs sm:text-sm md:text-[14px] font-inter font-weight-400 uppercase tracking-wider font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
