...
export const ReadingItemChecklist = ({
  ...
  return (
    <div className="space-y-3 rounded-lg">
      {items.map(
        ({ id, isCompleted, bookId, title, day, bookName, chapter, description }, index) => {
          const isMarked = isCompleted || localReadItems[id]

          return (
            <div
              key={id}
              className="text-primary-foreground flex items-center justify-between rounded-lg border p-4 transition-colors"
            >
              <div className="flex flex-1 items-center gap-4">
                <div
                  className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 ${isMarked ? 'border-green-600 bg-green-300/80' : 'border-gray-300'}`}
                >
                  {isMarked ? (
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-foreground dark:text-background-foreground font-medium">
                    {title ||
                      t('dayText', { day: day ?? index + 1, book: bookName ?? '', chapter })}
                  </p>
              ...
              <Link
                ...
                className="dark:text-accent-foreground flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-gray-800 transition-colors hover:bg-red-900 hover:text-white dark:hover:bg-red-900"
              >
