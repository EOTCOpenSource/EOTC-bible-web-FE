...
export function PlansExploreClient() {
  ...
  const wholeBible = useMemo(
    () => PLAN_TEMPLATES.filter((p) => p.tags?.includes('Whole Bible')),
    [],
  )
  const encouragement = useMemo(
    () => PLAN_TEMPLATES.filter((p) => p.tags?.includes('Encouragement')),
    [],
  )
    ...
    <div className="w-full bg-[#FBF8F2] dark:bg-neutral-900">
      ...
      <div className="mx-auto -mt-10 w-full max-w-6xl px-4">
        <div className="flex justify-center">
          <div className="inline-flex items-center gap-2 rounded-xl border border-black/10 bg-white/70 px-2 py-2 dark:border-neutral-700 dark:bg-neutral-800/70">
            {(['my-plans', 'explore', 'saved', 'completed'] as TabKey[]).map((k) => (
              <button
                ...
                className={[
                  ...
                  tab === k
                    ? 'border-[#392D2D] bg-[#392D2D] text-white'
                    : 'border-black/10 bg-white/70 text-[#1A1A19] hover:bg-white dark:border-neutral-600 dark:bg-neutral-700/70 dark:text-neutral-100 dark:hover:bg-neutral-600',
                ].join(' ')}
              >
                {t(`tabs.${k}`)}
              </button>
      ...
      <div id="explore" className="mx-auto w-full max-w-6xl space-y-12 px-4 py-10">
        {tab === 'my-plans' && (
          <section>
            <div className="flex items-end justify-between gap-4">
              <div>
                <h2 className="font-heading text-2xl text-[#1A1A19] dark:text-neutral-100">
                  {t('myPlansTitle')}
                </h2>
                <p className="mt-1 text-sm text-[#1A1A19DB] dark:text-neutral-400">
                  {t('myPlansDescription')}
                </p>
              </div>
              <Button asChild className="bg-[#4C0E0F] text-white hover:bg-[#3b0b0c]">
        ...
        {tab === 'saved' && (
          <section>
            ...
            {savedTemplates.length === 0 ? (
              <div className="mt-6 text-[#1A1A19DB] dark:text-neutral-400">{t('noSavedPlans')}</div>
            ) : (
        ...
        {tab === 'explore' && (
          <>
            <section className="w-full rounded-2xl border border-black/10 bg-white/60 p-6 dark:border-neutral-700 dark:bg-neutral-800/60">
              <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-12">
                <div className="lg:col-span-4">
                  <h2 className="font-heading text-xl text-[#1A1A19] dark:text-neutral-100">
                    {t('exploreDifferentTitle')}
                  </h2>
                  ...
                  <Button
                    asChild
                    className="mt-4 h-[38px] rounded-md border border-black/10 bg-white px-3 text-[#1A1A19] hover:bg-white dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-600"
                  >
