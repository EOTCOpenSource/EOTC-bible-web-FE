...
export default function PlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isFetching && !plan) {
    return <div className="flex items-center justify-center p-4">Loading plan details...</div>
  }

  if (!plan) {
    return <div className="flex items-center justify-center p-4">Plan not found.</div>
  }
  ...
  return (
    <div className="w-full space-y-6 p-4">
      <button
        onClick={handleBack}
        className="text-foreground dark:text-background flex items-center text-sm hover:text-gray-900 dark:hover:text-white"
      >
      ...
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Day {currentDayNumber} of {plan.durationInDays} -{' '}
          {currentDayData?.isCompleted ? 'Completed' : 'Not Completed'}
        </p>
      </div>
      ...
      <div>
        <h2 className="dark:bg-background dark:border-border mb-4 rounded-lg border border-gray-300 p-6 text-xl font-semibold text-black dark:text-white">
