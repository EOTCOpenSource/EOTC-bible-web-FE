  ...
  devtools((set, get) => ({
    ...
    markChapterRead: async (book, chapter) => {
      ...
      set((s) => {
        ...
        chapters[book] = Array.from(
          new Set([...(chapters[book] || []), chapter])
        );
        return {
      ...
      try {
        const res = await fetch(
          "https://mylocalbackend/api/v1/user/progress/mark",
          {
            method: "POST",
    ...
    setLastRead: async (v) => {
      ...
      try {
        const res = await fetch(
          "https://mylocalbackend/api/v1/user/progress/last-read",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
